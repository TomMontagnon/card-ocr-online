...
# def _unsharp(img, amount=1.0, radius=1.0):
#     blur = cv2.GaussianBlur(img, (0,0), radius)
#     return cv2.addWeighted(img, 1+amount, blur, -amount, 0)

# def _prep_small_text(bgr):
#     gray = cv2.cvtColor(bgr, cv2.COLOR_BGR2GRAY)
#     r = bgr[:, :, 2]
#     comb = cv2.max(gray, r)
#     up = cv2.resize(comb, None, fx=3.0, fy=3.0, interpolation=cv2.INTER_CUBIC)
#     clahe = cv2.createCLAHE(clipLimit=3.0, tileGridSize=(8,8))
#     up = clahe.apply(up)
#     up = _unsharp(up, amount=1.2, radius=1.0)
#     # adaptive gaussian (fallback portable)
#     bin_img = cv2.adaptiveThreshold(
#         up, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 31, 10
#     )
#     # Tesseract préfère souvent texte noir sur fond blanc
#     if np.mean(bin_img) < 127:
#         bin_img = 255 - bin_img
#     return bin_img

# def tesseract_small_text(bgr,
#                          lang="eng+fra",
#                          whitelist="ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789©:/+-.#&_(),;.%=<>|",
#                          psms=(7, 13, 6),
#                          min_conf=55):
#     """
#     OCR pour petits caractères. Renvoie une string.
#     ATTENTION: la whitelist ne doit contenir ni guillemets ni espaces.
#     """
#     if bgr is None or bgr.size == 0:
#         return ""

#     # 1) Prétraitement
#     bin_img = _prep_small_text(bgr)

#     # 2) Sanitize whitelist (retire guillemets/espaces au cas où)
#     whitelist = (whitelist or "")
#     blacklist = "".maketrans("", "", "\"'“”‘’ \t\r\n")
#     whitelist = whitelist.translate(blacklist)

#     best_txt, best_score = "", -1.0
#     for psm in psms:
#         # Pas de guillemets autour de la valeur de -c
#         config = f"--oem 1 --psm {psm} -c tessedit_char_blacklist= -c tessedit_char_whitelist={whitelist}"
#         try:
#             data = pytesseract.image_to_data(
#                 bin_img, lang=lang, config=config, output_type=pytesseract.Output.DICT
#             )
#         except ValueError:
#             # Fallback si shlex se plaint quand même: on retire la whitelist
#             config = f"--oem 1 --psm {psm} -c tessedit_char_blacklist="
#             data = pytesseract.image_to_data(
#                 bin_img, lang=lang, config=config, output_type=pytesseract.Output.DICT
#             )
...
#     if not best_txt:
#         # dernier essai: PSM 11 sans whitelist
#         config = "--oem 1 --psm 11"
#         best_txt = pytesseract.image_to_string(bin_img, lang=lang, config=config).strip()
